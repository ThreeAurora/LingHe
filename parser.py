...
4. 首字辅码（奇数长 n≥5）：首音节(2键) + 首字辅码(1键) + 剩余音节(每 2 键一个)，
   （yicui = 意(yi+c辅码)识(ui)），辅码附着首字。
...
def parse(code: str):
    ...
    # 模式 4：首字辅码（奇数长 n≥5）：首音节(2键)+首字辅码(1键)+剩余音节(每2键一个)
    if n % 2 == 1 and n >= 5:
        syls = (code[:2],) + tuple(code[i:i + 2] for i in range(3, n, 2))
        out.append((syls, ((0, code[2]),)))  # 辅码附首字

    return out


if __name__ == "__main__":
    for c in ["yiyi", "yiyit", "yiyic", "yityic", "yityi", "vhmusi", "ulpb", "vhmus", "yicui"]:
        print("%-7s ->" % c)
